...
async function checkAndCompleteReferrals(userId) {
    try {
        ...
        for (const referral of pendingReferrals) {
            ...
            // Award $1.50 bonus to the referrer
            const referrer = await User.findById(referral.referrer);
            if (referrer) {
                const bonusAmount = 1.50;
                referrer.balance += bonusAmount;
                referrer.referralBonusEarned += bonusAmount;
                referrer.referralBonusBalance += bonusAmount;
                await referrer.save();
                
                console.log(`💰 REFERRAL BONUS: Awarded $${bonusAmount} to ${referrer.username} for referring ${user.username}`);
            }
    ...
    try {
      ...
      return res.json({ 
        user: {
          ...
          referralBonusEarned: req.user.referralBonusEarned || 0, // Total referral bonuses earned
          referralBonusBalance: req.user.referralBonusBalance || 0, // Available referral bonus balance
...
app.get('/api/referral-bonus/stats', ensureAuthenticated, async (req, res) => {
  try {
    const user = await User.findById(req.user._id);
    if (!user) {
      return res.status(404).json({ error: 'User not found' });
    ...
    // Get total completed referrals for this user
    const totalReferrals = await Referral.countDocuments({ 
    ...
    res.json({
      success: true,
      totalEarned: user.referralBonusEarned || 0,
      availableBalance: user.referralBonusBalance || 0,
      totalReferrals: totalReferrals,
      bonusPerReferral: 1.50
    });
  } catch (error) {
    console.error('Error fetching referral bonus stats:', error);
    res.status(500).json({ error: 'Failed to fetch referral bonus stats' });
  }
});
